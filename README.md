...
主模型（执行者）只负责推理、输出精炼语义；便宜大碗的转译模型把最终
回复改写成雾铃澪（Kirisuzu Mio）的猫娘风格。执行者智商永远在线，
转译者负责「情商」，两者各司其职。
...
┌─────────────────┐          ┌──────────────────────────┐
│ 推理 / 工具调用   │          │ 文本风格迁移（不改信息）     │
│ 输出精炼语义     │─────────>│ 注入：转译者人格 + 长期记忆  │
│ 不角色扮演       │  transform│  + 映射词典（用户->主人）   │
└─────────────────┘   hook    └───────────┬──────────────┘
        ▲                                  │
...
2. 正常提问，回复自动变成猫娘风格（代码/数字/结论原样保留）。
3. 想看执行者原始输出：`/et raw` 直接贴出上一轮原文，不用重新问。

## 典型用法

```
/et remember 主人喜欢喵直接连句不加逗号   # 教转译者表达偏好（不影响执行者记忆）
/et persona 你是冷静的猫娘执事风格        # 换人格设定
/et raw                                  # 回看最近一轮执行者原文
...
3. `/et test` 走一遍真实转译管线。
4. 正常提问一次，观察回复是否变成猫娘风格；对比 `/et bypass` 后
   的原文输出。
...
- 串行等待两个模型是主要延迟来源；CLI 下感知为「思考完再变猫娘」。
